 ...
 * One fold implementation serves both history replay and live streaming.
 */
...
function blockText(blocks: Block[] | undefined): string {
  return (blocks ?? [])
    .map((b) => {
      switch (b.type) {
        case 'text':
...
/** Fold a tool result event onto an assistant message. */
