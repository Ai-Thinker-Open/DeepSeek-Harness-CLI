...
export class BridgeStore {
  ...
  listSessions(): OpenCodeSession[] {
    return [...this.sessions.values()].map((entry) => entry.session).sort((a, b) => b.time.updated - a.time.updated)
  }
  ...
  upsertSession(summary: SessionSummary): OpenCodeSession {
    ...
    const session: OpenCodeSession = {
      ...
      time: { created: existing?.session.time.created ?? Date.now(), updated: summary.updatedAt ?? Date.now() },
    }
  ...
  syncHistory(sessionId: string, history: HistoryEntry[]): void {
    ...
    state.messages = []
    state.parts = new Map()
    for (const entry of history) this.applyEvent(sessionId, entry.event)
  }
  ...
  applyEvent(sessionId: string, event: SessionEvent): void {
    ...
    if (event.type === 'user/message') {
      ...
      const message: OpenCodeMessage = { id: messageId, sessionID: sessionId, role: 'user', time: { created: event.time } }
      state.messages.push(message)
      if (content) this.addPart(state, messageId, { id: id('part'), sessionID: sessionId, messageID: messageId, type: 'text', text: content })
      this.touch(sessionId, event.time)
      this.emit({ type: 'message.updated', sessionID: sessionId, info: message })
      return
    }
    ...
    if (event.type === 'assistant/message') {
      ...
      const message: OpenCodeMessage = { id: messageId, sessionID: sessionId, role: 'assistant', time: { created: event.time } }
      state.messages.push(message)
      if (reasoning) this.addPart(state, messageId, { id: id('part'), sessionID: sessionId, messageID: messageId, type: 'reasoning', text: reasoning })
      if (content) this.addPart(state, messageId, { id: id('part'), sessionID: sessionId, messageID: messageId, type: 'text', text: content })
      for (const block of payload.content ?? []) {
        if (block.type === 'tool-call') this.addPart(state, messageId, this.toolPart(sessionId, messageId, block))
      }
      this.touch(sessionId, event.time)
      this.emit({ type: 'message.updated', sessionID: sessionId, info: message })
      return
    ...
    if (event.type === 'tool/result') {
      ...
      if (!target) return
      const part = (state.parts.get(target.id) ?? []).find((candidate): candidate is OpenCodeToolPart => candidate.type === 'tool' && candidate.tool === block.toolCallId)
      if (!part) return
      part.state = { ...part.state, status: block.isError ? 'error' : 'completed', output: textFromBlocks(block.content) }
      this.emit({ type: 'message.part.updated', sessionID: sessionId, part })
      return
    }
    ...
    if (event.type === 'session/title') {
      ...
      this.emit({ type: 'session.updated', sessionID: sessionId, info: state.session })
    }
  }
  ...
  applyProjection(sessionId: string, key: string, value: unknown): void {
    ...
    if (key === 'todos' && Array.isArray(value)) {
      ...
      this.emit({ type: 'todo.updated', sessionID: sessionId, todos: state.todos })
    }
  ...
  setStatus(sessionId: string, status: BridgeSessionState['status']): void {
    const state = this.ensure(sessionId)
    state.status = status
    this.emit({ type: 'session.status', sessionID: sessionId, status })
  }
  ...
  private ensure(sessionId: string): BridgeSessionState {
    ...
    if (!state) {
      state = {
        session: { id: sessionId, title: sessionId.slice(0, 12), directory: this.directory, time: { created: Date.now(), updated: Date.now() } },
        messages: [],
        parts: new Map(),
        todos: [],
        plan: { active: false },
  ...
  private addPart(state: BridgeSessionState, messageId: string, part: OpenCodePart): void {
    ...
    this.emit({ type: 'message.part.updated', sessionID: part.sessionID, part })
  }

  private toolPart(sessionId: string, messageId: string, block: DshBlock): OpenCodeToolPart {
    let input: unknown = {}
    try {
