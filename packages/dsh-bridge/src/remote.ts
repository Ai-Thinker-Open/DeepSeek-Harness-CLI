/**
 * DSH Typert Remote client for the human command plane.
 *
 * The legacy `HarnessClient` covers `session.*`; `commands.*` are Remote
 * methods carried on the same `/api` connection but addressed by the
 * two-segment method name and wrapped in `{ args: ... }`.
 */

import { HarnessClient } from './harness.ts'

export interface DshCommandDescriptor {
  name: string
  description: string
  input?: { hint: string }
}

export interface DshCommandExecution {
  commandId: string
  result: { kind: 'success' | 'error'; text?: string }
}

export class DshRemoteClient {
  constructor(
    private readonly client: HarnessClient,
    public timeoutMs = 60_000,
  ) {}

  async listCommands(sessionId: string): Promise<DshCommandDescriptor[]> {
    const result = await this.client.call<{ ok: true; value: DshCommandDescriptor[] } | { ok: false; error: unknown }>(
      'commands/list',
      { args: { agentId: sessionId } },
      AbortSignal.timeout(this.timeoutMs),
    )
    if (!('value' in result) || !Array.isArray(result.value)) return []
    return result.value
  }

  async executeCommand(sessionId: string, line: string): Promise<DshCommandExecution | undefined> {
    const result = await this.client.call<
      { ok: true; value: { commandId: string } | undefined } | { ok: false; error: unknown }
    >(
      'commands/execute',
      { args: { agentId: sessionId, line } },
      AbortSignal.timeout(this.timeoutMs),
    )
    if (!('value' in result) || !result.value) return undefined
    return {
      commandId: result.value.commandId,
      result: { kind: 'success' },
    }
  }
}
